...
class FrameAnalyser(
    ...
    val recognitionCounts = HashMap<String, Int>()
    val recognitionThreshold = 10
    var isCheck = true
            ...
            for (face in faces) {
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        addCountRecog(bestScoreUserName, cameraFrameBitmap)
    ...
    private fun addCountRecog(bestScoreUserName: String, bitmap: Bitmap) {
        if (bestScoreUserName != "Unknown"&& isCheck) {
            // Tăng số lần nhận diện
            val count = recognitionCounts.getOrDefault(bestScoreUserName, 0) + 1
            recognitionCounts[bestScoreUserName] = count

            // Kiểm tra ngưỡng và lưu ảnh
            if (count >= recognitionThreshold ) {
//            saveImage(cameraFrameBitmap, bestScoreUserName)
//            // Đặt lại đếm cho người này sau khi lưu ảnh
                sendImage(bitmap,bestScoreUserName)
                recognitionCounts[bestScoreUserName] = 0
                isCheck = false
            ...
            // Đặt lại tất cả các đếm nếu nhận diện là "Unknown"
            recognitionCounts.keys.forEach { key ->
                recognitionCounts[key] = 0
            }
        }
    }
